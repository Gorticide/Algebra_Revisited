// g++ -g completesquare4.cpp -std=c++11 -o cts
...
int extractFactor(int radicand);
...
if (d < 0)
{
  ...
  if (factor == 1)  {
    ...
    std::cout << " \nx = [";
    printFraction(&std::cout, (double)(-b/factor));
    std::cout << " + " << simplifyRadical(-d/(factor*factor))
              << " i ] / " << 2*a/factor;
    std::cout << " \nx = [";
    printFraction(&std::cout, (double)(-b/factor));
    std::cout << " -  " << simplifyRadical(-d/(factor*factor))
  ...
  std::cout << "\n\nReal Part : ";
  printFraction(&std::cout, (double)(-b/factor)/(double)(2*a/factor));
  std::cout << "\nImaginary Part : " << "[+/-] (";
  printFraction(&std::cout, (double)(factor/(2*a)));
  std::cout << ")*"<< simplifyRadical(-d/(factor*factor));
...
// Extracting Factor from radicand to reduce to simplest terms for posterity
int extractFactor(int radicand)  {
...
std::ostream &printFraction(std::ostream* os, const double target)  {
  ...
  // Of extreme significance *os rather than &os, const double, not &double;
...
/*
 * Version 4 will use extractFactor(radicand) before simplifyRadical()
 * so as to reduce to lowest terms even with radical in numerator!
 *
 * using struct fraction, and simplifying with Brute Force:
 * added printFraction(ostream* os, const double)
